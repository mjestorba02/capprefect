...
class RequestManager {
            ...
            <tr>
                ...
                <td>
                    <div class="table-actions">
                        ${request.status === 'pending' && auth.hasPermission('approve') ? `
                        ...
                        ` : ''}
                        ${(request.requestor_id === auth.getCurrentUser().id || auth.hasPermission('all')) ? `
    ...
    saveRequest() {
        ...
        if (this.editingRequest) {
            db.update('financial_requests', this.editingRequest.id, requestData);
            Utils.showNotification('Request updated successfully', 'success');
        } else {
            requestData.status = 'pending';
            db.insert('financial_requests', requestData);
